...
export class Ontodia extends Component<OntodiaProps, State> {
  ...
  private resolveElementTemplate = (types: string[]): Reactodia.ElementTemplate | undefined => {
  ...
  private getElementTemplate = (template: string): Reactodia.ElementTemplate => {
    return {
      ...
      renderElement: props => {
        ...
        const options = getElementTemplateContext(props, workspace);
        const data = props.element instanceof Reactodia.EntityElement ? props.element.data : undefined;
        const authoredContext = Reactodia.useAuthoredEntity(data, inAuthoringMode);
        if (inAuthoringMode) {
          return createElement(TemplateItem, {
            template: { source: template, options },
            componentMapper: (component) => mapTemplateComponent(component, authoredContext),
          });
        } else {
          return <TemplateItem template={{ source: template, options }} />;
        }
      },
    };
  };

  private resolveLinkTemplate = (linkTypeId: string): Reactodia.LinkTemplate | undefined => {
    const { edgeStyles, defaultEdgeStyle } = this.props;
